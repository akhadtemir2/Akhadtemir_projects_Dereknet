...
BOT_TOKEN = "8665265232:AAGQWyNdEBZOWszo4Gi3WOkg9BXMTD-DVYY"

OFFICE_LAT = 47.11150731320942
OFFICE_LON = 51.92043675195111
OFFICE_RADIUS_M = 100
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    faces = load_faces()
    if not faces:
        await update.message.reply_text("База сотрудников пуста. Обратитесь к администратору.", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END

    names = [faces[k]["display_name"] for k in faces]
    keyboard = [[name] for name in names]
    await update.message.reply_text("Выберите своё имя из списка:", reply_markup=ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True))
    return SELECT_NAME
...
async def handle_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if chosen.lower() not in display_names:
        await update.message.reply_text("Вас нет в базе. Обратитесь к администратору.", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END

    real_name = display_names[chosen.lower()]

    if already_checked_in(real_name):
        await update.message.reply_text("Вы уже отмечены сегодня.", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END

    context.user_data["name"] = real_name
    await update.message.reply_text("Отправьте своё фото для подтверждения личности.", reply_markup=ReplyKeyboardRemove())
    return VERIFY_FACE
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not ok:
        await update.message.reply_text(f"Верификация не пройдена: {msg}\n\nНачните заново — /start", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END

    location_btn = KeyboardButton("Отправить геолокацию", request_location=True)
    await update.message.reply_text("Личность подтверждена. Теперь отправьте геолокацию.", reply_markup=ReplyKeyboardMarkup([[location_btn]], one_time_keyboard=True, resize_keyboard=True))
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if distance > OFFICE_RADIUS_M:
        await update.message.reply_text(f"Вы находитесь слишком далеко от офиса ({dist_int} м).\nДопустимый радиус: {OFFICE_RADIUS_M} м.\n\nОтметка невозможна.", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END

    keyboard = [["Я пришёл"], ["Не приду"], ["В отпуске"]]
    await update.message.reply_text(f"Вы в офисе ({dist_int} м от точки). Выберите статус:", reply_markup=ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True))
...
async def handle_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    mapping = {"Я пришёл": "present", "Не приду": "absent", "В отпуске": "vacation"}

    if text not in mapping:
        await update.message.reply_text("Выберите один из вариантов.", reply_markup=ReplyKeyboardRemove())
        return SELECT_STATUS

    status_type = mapping[text]
    status_text = record_attendance(name, status_type)
    await update.message.reply_text(f"Готово, {name}.\nСтатус: {status_text}", reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END
...
async def post_init(app):
    ...
    print("Telegram бот запущен. Старые подключения сброшены.")
