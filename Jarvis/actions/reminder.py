...
from actions.proc import OEM_TEXT, out as _proc_out
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    # OEM_TEXT, not plain text=True: schtasks answers in the console code page
    # (cp866 here), and decoding that as UTF-8 killed subprocess's reader thread
    # — which silently turned stdout/stderr into None and destroyed the reason
    # for any failure. See actions/proc.py.
    result = subprocess.run(
        ["schtasks", "/Create", "/TN", task_name, "/XML", str(xml_path), "/F"],
        capture_output=True, **OEM_TEXT, **_CNW,
    )
    ...
    if result.returncode != 0:
        ...
        # proc.out() instead of (stderr or stdout).strip(): that idiom raises
        # AttributeError the moment either stream is None, which is precisely
        # what the decode crash used to produce — turning a readable scheduler
        # error into a bare "something went wrong".
        err = _proc_out(result) or f"schtasks exited with code {result.returncode}"
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("at"):
        ...
        result  = subprocess.run(
            ["at", at_time],
            input=cmd_str, capture_output=True, text=True, errors="replace",
        )
        if result.returncode == 0:
            return task_name
