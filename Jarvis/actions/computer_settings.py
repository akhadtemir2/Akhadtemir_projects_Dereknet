...
# Layout-independent shortcuts (raw virtual-key codes). key_combo maps
# ctrl→command on macOS automatically, so one call is correct on every OS and,
# unlike pyautogui.hotkey, it still reaches Ctrl+<letter> on Russian/Kazakh.
try:
    from actions import keyboard_util
except Exception:                       # frozen / flat-layout fallback
    import keyboard_util                 # type: ignore
...
def _close_focused_verified() -> str:
    """Close the foreground WINDOW (Alt+F4) and confirm it actually changed —
    no more blind keystroke + fake 'Done'. Alt+F4 is layout-independent."""
    if _OS != "Windows":
        close_app()
        return "Sent the close command to the active window."
    try:
        import ctypes
        ...
        if after != before or not user32.IsWindow(before):
            return "Closed the active window."
        return ("I sent the close command but the same window is still in front — "
                "it may need a save/confirm click, or nothing had focus.")
    except Exception as e:
        pyautogui.hotkey("alt", "f4")
        return f"Sent the close command (could not verify: {e})."
...
def find_on_page():
    keyboard_util.key_combo("ctrl", "f")
...
def copy():       keyboard_util.key_combo("ctrl", "c")
def paste():      keyboard_util.key_combo("ctrl", "v")
def cut():        keyboard_util.key_combo("ctrl", "x")
def undo():       keyboard_util.key_combo("ctrl", "z")

def redo():
    # macOS redo is Cmd+Shift+Z; Windows/Linux is Ctrl+Y (ctrl→command on mac).
    if _OS == "Darwin": keyboard_util.key_combo("ctrl", "shift", "z")
    else:               keyboard_util.key_combo("ctrl", "y")

def select_all(): keyboard_util.key_combo("ctrl", "a")
def save_file():  keyboard_util.key_combo("ctrl", "s")
...
def take_screenshot():
    if _OS == "Windows":
        keyboard_util.key_combo("win", "shift", "s")   # layout-independent snip
    elif _OS == "Darwin":
...
def toggle_wifi():
    if _OS == "Darwin":
        ...
        try:
            result = subprocess.run(["nmcli", "radio", "wifi"], capture_output=True, text=True, errors="replace")
...
- If NOTHING in the list plausibly matches the command, return {{"action": null, "value": null}}.
  Never force a match: inventing an action turns an unclear request into a real
  side effect on the user's machine (typing, closing windows, changing settings).
...
# Actions handled by explicit branches below rather than via ACTION_MAP.
_SPECIAL_ACTIONS = {
...
def _known_actions() -> set:
    return set(ACTION_MAP.keys()) | _SPECIAL_ACTIONS


def _resolve_action(action: str, description: str) -> tuple:
    """Turn a near-miss action name into a real one.

    Observed 2026-08-11: the model sent action='close' (with description
    "Close Obsidian"). ACTION_MAP has close_app/close_tab/close_window but no
    bare 'close', so the call died on "Unknown action" — even though
    _detect_action could have resolved it, because that rescue only ran when
    the action field was EMPTY. A model that helpfully fills in both fields
    was punished for it.

    Returns (resolved_action, note). resolved_action == "" means unresolved,
    and `note` is then an honest message that NAMES the near matches so the
    model can retry correctly instead of guessing again.
    """
    import difflib
    ...
    # 1) Unambiguous local synonym — no API call, no guessing.
    #    'wifi' → toggle_wifi. Only when EXACTLY one candidate exists.
    cands = sorted({
        k for k in known
        if k.startswith(action + "_") or k.endswith("_" + action) or action in k.split("_")
    })
    if len(cands) == 1:
        return cands[0], f"'{action}' → '{cands[0]}'"

    # 2) Obvious spelling slip — 'minimise' → 'minimize'. High bar, one winner.
    typo = difflib.get_close_matches(action, sorted(known), n=2, cutoff=0.8)
    if len(typo) == 1:
        return typo[0], f"'{action}' → '{typo[0]}' (spelling)"

    # 3) Only NOW may the intent detector guess — and only when the user
    #    actually said something. Its prompt orders it to "pick the closest
    #    action" and never to refuse, so feeding it a garbage token turns
    #    nonsense into a real side effect: 'qwertyzzz' resolved to type_text
    #    in testing, which would have typed on screen. A real description is
    #    genuine intent; a mangled action name is not.
    if description.strip():
        try:
            detected = _detect_action(description)
            cand = str(detected.get("action", "")).lower().strip().replace(" ", "_").replace("-", "_")
            if cand in known:
                return cand, f"'{action}' → '{cand}' (from description)"
        except Exception as e:
            print(f"[Settings] action resolution failed: {e}")

    # 4) Refuse honestly, naming the near matches so the model can retry
    #    correctly instead of guessing a second time.
    if not cands:
        cands = difflib.get_close_matches(action, sorted(known), n=5, cutoff=0.55)
    if cands:
        return "", (f"Unknown action '{action}'. Did you mean: "
                    f"{', '.join(cands[:6])}? Call again with the exact name.")
    return "", (f"Unknown action '{action}'. Valid examples: close_app, close_window, "
                f"volume_set, lock_screen, screenshot, minimize, toggle_wifi.")
...
def computer_settings(
    ...
    # Resolve a near-miss BEFORE anything downstream runs, so the explicit
    # branches (volume_set, type_text, …) and the danger check all see the
    # real action name.
    _resolved, _note = _resolve_action(action, description)
    if not _resolved:
        print(f"[Settings] {_note}")
        return _note
    if _note:
        print(f"[Settings] {_note}")
    action = _resolved

    # ── Targeted, VERIFIED close ────────────────────────────────────────────
    # "close <app>" must close THAT app's window and confirm it — not fire a
    # blind Ctrl+W (which only closes one tab in Win11's tabbed Explorer / a
    # browser) on whatever has focus and then lie "Done". A bare close with no
    # name closes the foreground window and verifies. close_tab stays Ctrl+W.
    if action in ("close_app", "close_window", "close"):
        target = (description or "").strip() or str(value or "").strip()
        try:
            from actions.open_app import close_app as _close_by_name, _CLOSE_VERBS_RE
            target = _CLOSE_VERBS_RE.sub("", target).strip()
            if target:
                result = _close_by_name(target)
                if player:
                    player.write_log(f"[Settings] close → {result}")
                print(f"[Settings] close '{target}' → {result}")
                return result
        except Exception as e:
            print(f"[Settings] close delegate failed, using focused close: {e}")
        result = _close_focused_verified()
        if player:
            player.write_log(f"[Settings] {result}")
        return result
