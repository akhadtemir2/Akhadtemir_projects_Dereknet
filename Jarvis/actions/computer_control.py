...
# Layout-independent keyboard shortcuts (raw virtual-key codes). Required because
# pyautogui.hotkey('ctrl','v') silently presses nothing on Russian/Kazakh layouts.
try:
    from actions import keyboard_util
except Exception:                       # frozen / flat-layout fallback
    import keyboard_util                 # type: ignore

# Keys keyboard_util can press regardless of active layout (raw VK codes).
_KU_KEYS = {
    "ctrl", "shift", "alt", "win", "enter", "delete", "backspace", "tab", "esc",
} | set("abcdefghijklmnopqrstuvwxyz0123456789")

def _norm_key(k: str) -> str:
    k = str(k).strip().lower()
    return {
        "control": "ctrl", "cmd": "ctrl", "command": "ctrl",
        "return": "enter", "del": "delete", "escape": "esc",
    }.get(k, k)
...
def _enter_text_layout_safe(text: str, interval: float = 0.03) -> bool:
    """Enter text WITHOUT depending on the active keyboard layout.

    Returns True if the reliable clipboard-paste path was used, False if it fell
    back to raw scan-code typing (which can mangle text under a mismatched
    layout) — the caller reports honestly which happened.

    keyboard_util.paste_text pastes via raw virtual-key codes (Ctrl+VK_V), which
    works on Russian/Kazakh layouts where pyautogui.hotkey('ctrl','v') silently
    presses nothing because 'v' isn't reachable on a Cyrillic layout — the exact
    bug that used to leave typed text (folder names, messages) empty.
    """
    _require_pyautogui()
    if text and keyboard_util.paste_text(text):
        return True
    if text:
...
def _type(text: str, interval: float = 0.03) -> str:
    ...
    time.sleep(0.2)
    ok = _enter_text_layout_safe(text, interval)
    preview = f"{text[:60]}{'…' if len(text) > 60 else ''}"
    if ok:
        return f"Typed: {preview}"
    return f"Typed via raw keys (clipboard unavailable — may be wrong on a non-Latin layout): {preview}"
...
def _smart_type(text: str, clear_first: bool = True) -> str:
    ...
    if clear_first:
        _clear_field()
        time.sleep(0.1)
    ok = _enter_text_layout_safe(text)
    preview = f"{text[:60]}{'…' if len(text) > 60 else ''}"
    if ok:
        return f"Smart-typed: {preview}"
    return f"Smart-typed via raw keys (clipboard unavailable — may be wrong on a non-Latin layout): {preview}"
...
def _hotkey(*keys) -> str:
    ...
    # Layout-independent path for the editing combos pyautogui can't press on a
    # Russian/Kazakh layout (Ctrl+V/A/C/X/Z/F). Anything else → pyautogui.
    if norm and all(k in _KU_KEYS for k in norm):
        keyboard_util.key_combo(*norm)
        return f"Hotkey: {'+'.join(norm)}"
...
def _clipboard_paste(text: str) -> str:
    _require_pyautogui()
    if keyboard_util.paste_text(text):
...
def _clear_field() -> str:
    _require_pyautogui()
    keyboard_util.key_combo("ctrl", "a")   # layout-independent select-all
    time.sleep(0.1)
...
def _screen_find(description: str) -> tuple[int, int] | None:
    """AI element finder → coordinates in pyautogui's cursor space.

    The model answers in 0-1000 NORMALIZED coordinates (Gemini's native
    pointing convention — far more accurate than free-form pixel guesses),
    which are then projected onto pyautogui.size(). This stays correct even
    when DPI scaling makes the screenshot resolution differ from the cursor
    space — the historic cause of offset clicks on this 125%-scaled machine.
    """
    ...
    try:
        ...
        prompt = (
            "Locate the UI element on this screenshot described as: "
            f"'{description}'. "
            "Reply with ONLY the element's center as x,y in NORMALIZED "
            "0-1000 coordinates (x: 0=left edge, 1000=right edge; "
            "y: 0=top, 1000=bottom of the image). Example: 512,340 "
            "If the element is not visible, reply: NOT_FOUND"
        )

        model = _load_config().get("screen_find_model", "") or "gemini-flash-latest"
        ...
        if match:
            ...
            if x <= 1000 and y <= 1000:            # normalized (the contract)
                sx, sy = x / 1000.0 * sw, y / 1000.0 * sh
            else:                                   # model answered in image pixels
                iw, ih = img.size
                sx, sy = x * sw / max(iw, 1), y * sh / max(ih, 1)
            return (int(min(max(sx, 0), sw - 1)), int(min(max(sy, 0), sh - 1)))
