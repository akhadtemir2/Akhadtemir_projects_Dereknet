...
# Names that allow sandbox escape or code loading even without imports.
_FORBIDDEN_CALLS = {
    "eval", "exec", "compile", "getattr", "setattr", "delattr",
    "globals", "locals", "vars", "open", "input", "breakpoint",
    "memoryview", "type", "__import__", "help", "exit", "quit",
    "classmethod", "staticmethod", "super",
}


def _code_is_safe(code: str) -> tuple[bool, str]:
    """AST allowlist gate for LLM-generated code before exec.

    A restricted __builtins__ is not enough on its own: CPython introspection
    (e.g. ().__class__.__bases__[0].__subclasses__()) can reach os/subprocess
    with no imports and no __import__. So we statically reject, before exec:
      • any import statement,
      • any dunder attribute/name access (__class__, __globals__, __subclasses__…),
      • calls to eval/exec/compile/getattr/open/__import__/etc.
    """
    try:
...
def _execute_generated_code(code: str, player=None) -> str:
    ...
    ok, reason = _code_is_safe(code)
    if not ok:
        print(f"[Desktop] Blocked unsafe code: {reason}")
        return "This action cannot be performed safely."
