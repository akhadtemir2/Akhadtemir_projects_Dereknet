...
def _nearby_matches(wanted: str, limit: int = 5) -> list:
    """Find real files/folders whose name is close to `wanted`.

    A name spoken aloud is approximate: on 2026-08-11 "СПС накладная" never
    matched the real `сфс_накладная`, and on 2026-08-12 a name spelled out
    letter-by-letter arrived as "D-E-R-E-K-N-E-T". Rather than answering a
    flat "Not found", look at what actually exists and offer it.
    """
    try:
        from core import name_match
    except Exception:
        return []
    ...
    for root in roots:
        ...
        try:
            ...
            continue
    ranked = name_match.rank(wanted, entries, key=lambda p: p.name, limit=limit)
...
def delete_file(path: str, name: str = "") -> str:
    try:
        ...
        if not _is_safe_path(target):
            return f"Access denied: {target}"
        if not target.exists():
            # Never auto-delete a guess: name the real near matches and let
            # the caller pick. Silently redirecting a delete to a
            # similar-looking path is how a fuzzy matcher destroys the wrong
            # thing while "fixing" a lookup.
            wanted = name or Path(path).name or str(path)
            near = _nearby_matches(wanted)
            # If the real match lives in the Obsidian vault, this tool is the
            # wrong door — say so explicitly. prompt.txt does mention it, but
            # only as a parenthesis at the end of a line that owns the words
            # "удали" and "папку", so the model picks file_controller every
            # time (reproduced 3/3 on 2026-08-12). A redirect in the RESULT is
            # read; a caveat in the prompt is not.
            in_vault = [p for p in near if "obsidian vault" in str(p).lower()]
            if in_vault:
                rel = str(in_vault[0]).split("Obsidian Vault", 1)[-1].strip("\\/")
                return (f"Not found on disk: {target.name}\n"
                        f"But '{rel}' exists in the user's OBSIDIAN VAULT. "
                        f"file_controller is the wrong tool for vault content — "
                        f"call obsidian_note instead: "
                        f"obsidian_note(action='delete', folder='{rel}').")
            if near:
                listed = "\n".join(f"  • {p}" for p in near)
                return (f"Not found: {target.name}\nThese real paths look close:\n"
                        f"{listed}\nCall again with the exact path above.")
            return f"Not found: {target.name}"
...
def file_controller(
    ...
    # Default to the whole user profile, not just the Desktop. With "desktop"
    # here, `find` could never see Documents/ — including the Obsidian vault —
    # so every question about the user's own notes answered "not found" even
    # with a perfect name (observed 2026-08-11). find_files() itself already
    # defaults to "home"; this line was silently overriding it.
