...
def _active_monitor(sct) -> dict:
    """Pick the monitor the user is actually looking at — the one holding the
    foreground window — instead of blindly grabbing the primary display.

    On multi-monitor setups the old 'always monitors[1]' logic captured the
    primary screen even when the user's chat/app was on a second monitor, so
    JARVIS truthfully described the *wrong* screen ("I see servers…"). Falls
    back to the primary monitor if the foreground window can't be located.
    """
    monitors = sct.monitors                      # [0] = all combined, [1..n] = real screens
    physical = monitors[1:] if len(monitors) > 1 else monitors[:1]

    if len(physical) > 1 and sys.platform == "win32":
        try:
            import ctypes
            ...
            print(f"[Vision] active-monitor detect failed, using primary: {e}")
