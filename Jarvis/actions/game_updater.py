    ...
    try:
        if is_windows():
            out = subprocess.run(["tasklist", "/FI", "IMAGENAME eq steam.exe"],
                                 capture_output=True, text=True, errors="replace", **_CNW).stdout
    ...
    try:
        if is_windows():
            out = subprocess.run(
                ...
                capture_output=True, text=True, errors="replace", **_CNW
            ).stdout
...
def _schedule_windows(hour: int, minute: int) -> str:
    ...
    for extra in (["/RL", "HIGHEST", "/RU", "SYSTEM"], []):
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, errors="replace", **_CNW)
...
def _get_schedule_status() -> str:
    ...
    try:
        result = subprocess.run(["crontab", "-l"], capture_output=True, text=True, errors="replace")
