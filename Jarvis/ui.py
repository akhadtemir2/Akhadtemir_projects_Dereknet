...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "J.A.R.V.I.S", parent=None):
        ...
        # ── Adaptive frame rate (the #1 heat fix) ───────────────────────────
        # The old HUD repainted complex vector graphics at a flat 60fps forever,
        # even while idle or minimised — that is what spins the fans up. Now the
        # frame rate follows activity + visibility (see _desired_interval), and
        # the static background is cached instead of redrawn every frame.
        self._power_saver    = bool(_read_full_config().get("hud_power_saver", True))
        self._visible_active = True
        self._bg_px: QPixmap | None = None
        self._bg_size        = (0, 0)
        self._cur_interval   = 16
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
        self._tmr.start(self._cur_interval)

    def _desired_interval(self) -> int:
        """Timer interval in ms based on activity + visibility (lower = more fps)."""
        if not self._power_saver:
            return 16
        if not self._visible_active:
            return 250          # minimised/hidden → ~4fps, negligible GPU load
        if self.speaking:
            return 16           # 60fps only while actually speaking
        if self.muted:
            return 66           # ~15fps when muted/idle
        return 40               # ~25fps listening/idle — smooth, far cooler

    def apply_power(self) -> None:
        iv = self._desired_interval()
        if iv != self._cur_interval:
            self._cur_interval = iv
            self._tmr.start(iv)

    def set_visible_active(self, active: bool) -> None:
        if active != self._visible_active:
            self._visible_active = active
            self.apply_power()

    def set_power_saver(self, on: bool) -> None:
        self._power_saver = bool(on)
        self.apply_power()

    def _blit_background(self, p: "QPainter", W: int, H: int) -> None:
        """Draw the static background (solid fill + grid dots) from a cached
        pixmap. Rebuilt only when the widget size changes — avoids an
        O(W*H/48^2) drawPoint loop on every single frame."""
        if self._bg_px is None or self._bg_size != (W, H):
    ...
    def _step(self):
        ...
        if self._blink_tick >= 38:
            ...
            self._blink_tick = 0
        self.update()
        # Re-evaluate the frame rate each tick so it tracks state changes
        # (speaking / muted / minimised) without hooking every setter.
        self.apply_power()
    ...
    def paintEvent(self, _):
        ...
        # Static background (solid fill + grid dots) → cached pixmap, drawn once.
        self._blit_background(p, W, H)
...
class FileDropZone(QWidget):
    ...
    def _animate(self):
        # Only spend paint cycles while the user is actually interacting — an
        # idle drop-zone must not repaint its marching-ants border 25×/sec forever.
...
class MainWindow(QMainWindow):
    ...
    def changeEvent(self, event):
        # Pause the HUD animation when the window is minimised — the single
        # biggest idle-heat win, since JARVIS lives minimised while you work.
        super().changeEvent(event)
        if event.type() == QEvent.Type.WindowStateChange:
            minimised = bool(self.windowState() & Qt.WindowState.WindowMinimized)
            try:
                self.hud.set_visible_active(not minimised)
            except Exception:
                pass
    ...
    def hideEvent(self, event):
        ...
        try:
            self.hud.set_visible_active(False)
        except Exception:
            pass
    ...
    def showEvent(self, event):
        ...
        try:
            self.hud.set_visible_active(True)
        except Exception:
            pass
    ...
    def _apply_state(self, state: str):
        ...
        self.hud.apply_power()   # ramp fps up/down immediately on state change
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        # Merge into any existing config instead of overwriting — re-running setup
        # must never wipe other settings (assistant_name, user_name, obsidian_*,
        # ambient_vision_*, etc.).
        data: dict = {}
        if API_FILE.exists():
            try:
