...
_LOGIN_MAX_FAILS    = 8     # failed attempts from one IP before lockout
_LOGIN_LOCKOUT_SECS = 300   # lockout window after threshold is hit
...
def _auto_firewall_enabled() -> bool:
    """Opt-in: only auto-modify the OS firewall when the user explicitly sets
    `dashboard_auto_firewall: true` in config/api_keys.json. Default is OFF —
    silently opening inbound ports / self-elevating is a security risk."""
    try:
        import json as _json
        with open(BASE_DIR / "config" / "api_keys.json", "r", encoding="utf-8") as f:
            return bool(_json.load(f).get("dashboard_auto_firewall", False))
    except Exception:
        return False
...
# ── AES-256-CBC + HMAC-SHA256 (Encrypt-then-MAC), key from PBKDF2 ──────────────
_AES_SALT     = b'JARVIS-DASHBOARD-v1'
_PBKDF2_ITERS = 100_000


def _derive_key(session_key: str) -> bytes:
    """PBKDF2-HMAC-SHA256(sessionKey, salt) → 64 bytes.

    [0:32] = AES-256 encryption key, [32:64] = HMAC-SHA256 key.
    Key stretching (100k iters) makes offline brute-force of the short session
    PIN expensive instead of a single hash away."""
    return hashlib.pbkdf2_hmac(
        'sha256', session_key.encode('utf-8'), _AES_SALT, _PBKDF2_ITERS, dklen=64
    )


def _decrypt_cbc(key_material: bytes, enc_b64: str) -> str:
    """Verify-then-decrypt base64(IV[16] ‖ ciphertext ‖ HMAC[32]).

    Encrypt-then-MAC: the HMAC over IV‖ciphertext is checked in constant time
    BEFORE any decryption/unpadding, which removes the CBC padding-oracle
    exposure (a wrong padding is now indistinguishable from a wrong MAC)."""
    import hmac as _hmac
    from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
    from cryptography.hazmat.primitives import padding as sym_pad
    enc_key, mac_key = key_material[:32], key_material[32:64]
...
def _ensure_network_access(port: int) -> None:
    ...
    # Opt-in only — never silently self-elevate or open ports by default.
    if not _auto_firewall_enabled():
        print(f"[Dashboard] Auto firewall setup is OFF (safe default).")
        print(f"[Dashboard] If phones can't connect, allow port {port}/tcp manually,")
        print(f"[Dashboard] or set \"dashboard_auto_firewall\": true in config/api_keys.json.")
        return
    ...
    if sys.platform == "win32":
        ...
        # Build a .bat file — netsh only. We never change the network category
        # (Public→Private) automatically: that would broaden exposure of the
        # whole machine without informed consent.
        bat_lines = ["@echo off"]
        if need_port:
            bat_lines.append(
    ...
    if sys.platform == "darwin":
        ...
        try:
            r = subprocess.run(
                [fw_ctl, "--getglobalstate"], capture_output=True, text=True, errors="replace", timeout=5,
            )
            ...
            listed = subprocess.run(
                [fw_ctl, "--listapps"], capture_output=True, text=True, errors="replace", timeout=5,
            )
    ...
    try:  # ufw
        r = subprocess.run(["ufw", "status"], capture_output=True, text=True, errors="replace", timeout=5)
    ...
    try:  # firewalld
        r = subprocess.run(
            ["firewall-cmd", "--state"], capture_output=True, text=True, errors="replace", timeout=5,
        )
...
def _is_tailscale_ip(ip: str) -> bool:
    """Tailscale assigns addresses in 100.64.0.0/10 (the CGNAT range)."""
    try:
        a, b = ip.split(".")[:2]
...
def _tailscale_ip() -> str | None:
    """The machine's Tailscale IP, so the dashboard is reachable FROM ANYWHERE
    over the private tailnet (not just the same WiFi). None if Tailscale is off."""
    # Primary: enumerate local interfaces — no subprocess, instant.
    try:
        for info in socket.getaddrinfo(socket.gethostname(), None, socket.AF_INET):
            ip = info[4][0]
        ...
        pass
    # Fallback: ask the Tailscale CLI.
    import subprocess
    for exe in (r"C:\Program Files\Tailscale\tailscale.exe",
        ...
        try:
            ...
            for line in (out.stdout or "").splitlines():
                if _is_tailscale_ip(line.strip()):
                    return line.strip()
        except Exception:
            continue
    return None
...
def _preferred_host() -> str:
    """Address the dashboard advertises to phones (QR / manual URL). Prefers,
    in order: an explicit config override → the Tailscale IP (reachable from
    anywhere) → the LAN IP (same-WiFi only)."""
    try:
        cfg = _json.load(open(BASE_DIR / "config" / "api_keys.json", encoding="utf-8"))
        override = (cfg.get("dashboard_host") or "").strip()
        if override:
            return override
    except Exception:
        pass
...
class DashboardServer:
    ...
    def __init__(self):
        # Prefer the Tailscale address so the phone reaches JARVIS from ANYWHERE
        # (private tailnet), falling back to the LAN IP for same-WiFi use.
        self._ip                          = _preferred_host()
        ...
        self._login_fails: dict[str, tuple[int, float]] = {}  # ip → (fail_count, lockout_until)
    ...
    def _login_locked_for(self, ip: str) -> float:
        """Remaining lockout seconds for this IP, or 0.0 if not locked."""
        _, until = self._login_fails.get(ip, (0, 0.0))
        remaining = until - time.time()
        return remaining if remaining > 0 else 0.0

    def _record_login_fail(self, ip: str) -> None:
        fails, _ = self._login_fails.get(ip, (0, 0.0))
        fails += 1
        if fails >= _LOGIN_MAX_FAILS:
            self._login_fails[ip] = (0, time.time() + _LOGIN_LOCKOUT_SECS)
        else:
            self._login_fails[ip] = (fails, 0.0)
    ...
    def _build_app(self) -> "FastAPI":
        ...
        async def login(req: Request):
            ...
            locked = self._login_locked_for(client_ip)
            if locked > 0:
                return JSONResponse(
                    {"ok": False, "error": f"Too many attempts — locked for {int(locked)}s"},
                    status_code=429,
                )
            body    = await req.json()
        ...
        async def auto_login(req: Request, key: str = ""):
            ...
            client_ip = req.client.host if req.client else "unknown"
            if self._login_locked_for(client_ip) > 0:
                return HTMLResponse(
                    "<h2>Too many attempts</h2><p>Try again in a few minutes.</p>",
                    status_code=429,
                )
            now = time.time()
...
<p>Connecting to JARVIS…</p>
    ...
    async def _serve_alias(self) -> None:
        ...
        cfg = uvicorn.Config(
            self.app, host=self._ip, port=PORT + 1, log_level="warning",
    ...
    async def serve(self) -> None:
        ...
        cfg = uvicorn.Config(
            self.app, host=self._ip, port=PORT, log_level="warning",
            **({"ssl_keyfile": str(ssl_key), "ssl_certfile": str(ssl_cert)} if use_ssl else {}),
        )

        proto = "https" if use_ssl else "http"
        _via = ("Tailscale — reachable from ANYWHERE"
                if _is_tailscale_ip(self._ip) else "LAN — same WiFi only")
        print(f"[Dashboard] {proto}://{self._ip}:{PORT}  ({_via})")
